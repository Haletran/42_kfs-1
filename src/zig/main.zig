...
fn init_term() void {
    ...
    for (0..v.SCROLLBACK_SIZE * VGA_WIDTH) |i| {
        v.scrollback_buffer[i] = utils.vga_entry(' ', v.terminal_color);
    }
    v.scrollback_position = 0;
    v.scroll_offset = 0;
}

fn scroll_down(page_key: bool) void {
    if (page_key and v.scroll_offset > 0) {
        v.scroll_offset -= 1;
        render_from_scrollback();
        return;
    }

    if (v.terminal_row >= VGA_HEIGHT) {
        const scroll_line = v.scrollback_position % v.SCROLLBACK_SIZE;
        const scroll_start = scroll_line * VGA_WIDTH;

        for (0..VGA_WIDTH) |x| {
            v.scrollback_buffer[scroll_start + x] = v.buffer[x];
        }

        v.scrollback_position += 1;
...
fn scroll_up(page_key: bool) void {
    if (page_key) {
        if (v.scroll_offset < v.scrollback_position and v.scroll_offset < v.SCROLLBACK_SIZE - VGA_HEIGHT) {
            if (v.scroll_offset == 0) {
                for (0..VGA_HEIGHT * VGA_WIDTH) |i| {
                    v.saved_screen[i] = v.buffer[i];
                }
            }
...
fn render_from_scrollback() void {
    if (v.scroll_offset == 0) {
        for (0..VGA_HEIGHT * VGA_WIDTH) |i| {
            v.buffer[i] = v.saved_screen[i];
        }
        return;
    }

    const start_line = if (v.scrollback_position > v.scroll_offset)
        v.scrollback_position - v.scroll_offset
    else
        0;

    for (0..VGA_HEIGHT) |y| {
        const line_to_show = start_line + y;

        if (line_to_show < v.scrollback_position) {
            const scroll_line = line_to_show % v.SCROLLBACK_SIZE;
            const scroll_start = scroll_line * VGA_WIDTH;

            for (0..VGA_WIDTH) |x| {
                v.buffer[y * VGA_WIDTH + x] = v.scrollback_buffer[scroll_start + x];
            }
        } else {
            const saved_line = line_to_show - v.scrollback_position;
            if (saved_line < VGA_HEIGHT) {
                const saved_start = saved_line * VGA_WIDTH;
                for (0..VGA_WIDTH) |x| {
                    v.buffer[y * VGA_WIDTH + x] = v.saved_screen[saved_start + x];
                }
            }
        }
    }
